...
def load_models(args):
    ...
    if "race" in args.meta.task:
        ...
        editor = Editor(editor_tokenizer_wrapper, editor_tokenizer, 
                ...
                length_penalty=args.generation.length_penalty, 
                verbose=False, 
                dataset_reader = get_dataset_reader(args.meta.task, predictor),
...
def run_edit_test(args):
    ...
    inputs, labels = dr.get_inputs(args.meta.dataset_dir, return_labels=True)
    ...
    with open(out_file, "w") as csv_file:
        ...
        for idx, i in tqdm(enumerate(input_indices), total=len(input_indices)):
            ...
            try:
                ...
                logger.info(wrap_text(f"===========>Original output: {output['original_input']}"))
                logger.info(wrap_text(f"===========>Counterfactual output: {output['counterfactual_input']}"))

                # print(f"=>Original output: {output['original_input']}")
                # print(f"=>Counterfactual output: {output['counterfactual_input']}")

            except Exception as e:
                logger.info(f"skipping index={idx}: No counterfactual examples. ")
                error = True
                sorted_list = []
                continue
