...
# def load_data(args, configs, tokenizer, accelerator):
#     """
#     # Get the datasets: you can either provide your own CSV/JSON/TXT training and evaluation files (see below)
#     # or just provide the name of one of the public datasets available on the hub at https://huggingface.co/datasets/
#     # (the dataset will be downloaded automatically from the datasets Hub).
...
#         data_files = {}
#         dataset_args = {}
#         data_train_file = os.path.join(configs["save_dirs"]["root_dir"], configs["save_dirs"]["data_train_file"])
#         data_validation_file = os.path.join(configs["save_dirs"]["root_dir"], configs["save_dirs"]["data_validation_file"])
#         data_test_file = os.path.join(configs["save_dirs"]["root_dir"], configs["save_dirs"]["data_test_file"])

#         if data_train_file is not None:
#             data_files["train"] = data_train_file
#         if data_validation_file is not None:
#             data_files["validation"] = data_validation_file
#         if data_test_file is not None:
#             data_files["test"] = data_test_file

#         extension = configs["save_dirs"]["data_train_file"].split(".")[-1]
...
#         raw_datasets = load_dataset(extension, data_files=data_files, **dataset_args)
#         print(f"===raw_datasets contains: {raw_datasets.keys()}")
...
#         ###### We drop the small remainder, we could add padding if the model supported it instead of this drop, you cancustomize this part to your needs.
#         if total_length >= block_size:
#             total_length = (total_length // block_size) * block_size
#         ###### Split by chunks of max_len.
#         result = {
...
#     # DataLoaders creation:
#     train_dataloader = DataLoader(
#         train_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=args.per_device_batch_size
#     )
#     eval_dataloader = DataLoader(
#         eval_dataset, collate_fn=default_data_collator, batch_size=args.per_device_batch_size
#     )
#     test_dataloader = DataLoader(
#         test_dataset, collate_fn=default_data_collator, batch_size=args.per_device_batch_size
#     )
