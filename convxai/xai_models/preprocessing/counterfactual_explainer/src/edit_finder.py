...
#     def extract(self, sents, padding=False):
#         results, pad_lens = [], []
#         for s in sents:
#             tokens = self.tokenizer.encode(s, add_special_tokens=False)
#             results.append(tokens)
#             pad_lens.append(len(tokens))

#         # results = [self.tokenizer.encode(s, add_special_tokens=False) for s in sents]

#         # turn to matrix with padding
#         if padding:
#             pad_length = np.max(pad_lens)
#             matrix = np.ones([len(results), pad_length], dtype=np.int32) * self.pad_id
#             for i, res in enumerate(results):
#                 length = min(len(res), pad_length)
...
def get_scores(predictor, instance_candidates, contrast_pred_idx,  k = None):
    ...
    """instance_candidates: [' cells , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ', , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' naevity , lack conventional endoplasmic reticulum ( ER ) or Golgi protein marker
        s ,', ' background , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' background. , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' protein , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
        ' primary , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' data , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' HE , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' background. primary
        protein markers , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' ER , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,']
    """
    # Get predictions
    with torch.no_grad():
        ...
        """ outputs[1] =
        tensor([[ 1.2595e+00, -1.9495e-01,  1.0475e-01,  3.7563e-01, -1.0167e+00],
        [ 1.0369e+00, -3.0099e-01,  4.8703e-02,  5.1337e-01, -8.3230e-01],
        [ 6.9269e-01,  4.2617e-01,  1.7886e-01,  7.7577e-01, -1.7564e-01],
        [ 1.1918e+00,  2.3755e-01, -1.4166e-01,  2.4652e-01, -9.1396e-01],
        [ 1.5334e+00, -1.2241e-01, -6.7751e-02,  2.4098e-01, -8.3001e-01],
        [ 7.6316e-01,  6.7022e-02,  3.0636e-01,  7.6342e-01, -3.8625e-01],
        [ 8.1952e-01,  3.1431e-01,  1.2477e-01,  5.3455e-01, -3.0750e-01],
        [ 1.1010e+00, -4.3491e-01,  2.4065e-03,  6.2918e-01, -8.8596e-01],
        [ 3.7818e-02,  4.8005e-01,  5.0541e-01,  8.8614e-01, -5.0099e-02],
        [ 9.2414e-01, -2.4304e-02,  3.9153e-01,  5.2083e-01, -2.9491e-01],
        [ 1.0626e+00,  4.0889e-01,  1.4400e-01,  3.1699e-01, -6.3024e-01],
        [ 1.3127e+00,  1.5473e-04,  7.2274e-02,  3.1264e-01, -7.4968e-01],
        [ 3.8299e-01,  3.2099e-01,  4.5118e-01,  8.4111e-01, -1.9606e-01],
        [ 9.3264e-01, -8.0405e-02,  1.7543e-01,  8.0281e-01, -4.4864e-01]],
       device='cuda:0')   shape = (10,20)
        """
        outputs = {
...
class EditFinder():
    ...
    def __init__(
        ...
        self.predictor.model = predictor.model.to(device)
        self.editor = editor
    ...
    def run_edit_round(
        ...
        """
            input_cands:       [' text, , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' input , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' ER , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' background. , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' background , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ', a rare type , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' p , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' low, lack conventional marker genes , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' negative , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', ' A cell , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,']
            editable_seg_cands [' text, , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' input , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' ER , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' background. , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' background , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ', a rare type , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' p , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' low, lack conventional marker genes , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' negative , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,', 
                                ' A cell , lack conventional endoplasmic reticulum ( ER ) or Golgi protein markers ,']
        """
