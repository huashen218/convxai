...
def get_datasets(predictor, dr, masker, data_dir, train_inputs, val_inputs, 
    ...
    # Else, create data by calling create_inputs() function in dataset.py
        ...
        if args.meta.task == "race":
            ...
            ### sample 10% dataset for training.
            sub_sample_size = int(len(train_inputs)*0.01)

            sub_train_dataset_index = np.random.choice(np.arange(len(train_inputs)), sub_sample_size)
            train_inputs = list(np.array(train_inputs)[sub_train_dataset_index])
            train_labels = list(np.array(train_labels)[sub_train_dataset_index])

            sub_sample_size_val = int(len(val_inputs)*0.01)
            sub_val_dataset_index = np.random.choice(np.arange(len(train_inputs)), sub_sample_size_val)
            val_inputs = list(np.array(val_inputs)[sub_val_dataset_index])
            val_labels = list(np.array(val_labels)[sub_val_dataset_index])
            print(f"Val Editor dataset Size: #train_inputs = {len(train_inputs)}, #val_inputs = {len(val_inputs)}")
...
def get_task_data(args, dr):
    ...
    elif args.meta.task == "coda19":
        strings, labels = dr.get_inputs(args.meta.dataset_dir, return_labels=True)
