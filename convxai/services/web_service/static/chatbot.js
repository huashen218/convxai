...
/*****************************************/
// Initialize the Conversations + Show the Reviews
/*****************************************/
...
function initiateConversation() {
    ...
    setTimeout(function(){
        //document.querySelector( ".chatBot .chatBody .chatSession" ).lastChild.scrollIntoView(false);
    },1)
    // selected_texts = $(".predict-selected").text();
...
chatBotSendButton.addEventListener("click", (event)=> {
    ...
    // Identify writing inputs and user explain input;
    inputMessage    = chatBotTextArea.value

    var selected_ids = [];
    console.log("===>>>selected_ids", selected_ids);
    console.log("$('.predict-selected')", $(".predict-selected"));
    ...
    var selected_texts = [];
    console.log("===>>>selected_texts", selected_texts);
    selected_ids.forEach(function(item, index) {
        // selected_texts.push($('[category=predicts]').filter('[id='+item+']').text());
    ...
    //Scroll handling for button clicked from chat history
...
socket.on("conv_response", function(data) {
    ...
    // remove the typing...
    var allwaitAnimates      = document.getElementsByName("wait-animate")
    // if (typeof(allwaitAnimates) != 'undefined') {
    ...
    // get sentence index of user's selection
    writingIndex = data.writingIndex
    ...
    // return Review Comments -- ["ACL", "CHI", "ICLR"]
    ...
    // check variables
        ...
        // var typeOfContainer = "reply"
        // createContainer(typeOfContainer)
        console.log("chatBotReplysAll", chatBotReplysAll)
...
function createContainer( typeOfContainer ) {
    ...
    switch( textClass ) {
        ...
        case "ask-for-xai"    :
            ...
            // btnYes.setAttribute( "class" , "reply xaiquestionButton" )
            // btnYes.setAttribute( "type" , "button" ) //
            // btnYes.setAttribute( "id" , "[GlobalWhy]" ) //
            // btnYes.innerHTML = "Explanations about the model and data."

            var btnNO                     = document.createElement("button");
            btnNO.setAttribute( "class" , "reply xaiquestionButton" )
            btnNO.setAttribute( "type" , "button" ) //chi
            btnNO.setAttribute( "id" , "[InstanceWhy]" ) //
            // btnNO.innerHTML = "Select and explain the sentence review"
            btnNO.innerHTML = "Explain sentence review"



            lastReplyContainer.appendChild( newReply )
            newReply.appendChild( breakline )
            // newReply.appendChild( btnYes )
            newReply.appendChild( btnNO )
            // btnYes.onclick = handleExplanationButtonClicked;
        ...
        case "global-explanation"    :
            ...
            btnData.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            ...
            btnModel.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            ...
            qualityScore.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            ...
            aspectLabel.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            aspectLabel.setAttribute( "type" , "button" ) //
            aspectLabel.setAttribute( "id" , "[Aspect Distribution]" ) //
            // aspectLabel.innerHTML = "How are the structure labels distributed?"
            ...
            lastReplyContainer.appendChild( newReply )
            // newReply.appendChild( globalReply )
        ...
        case "local-explanation"    :
            ...
            btnConfidence.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            btnConfidence.setAttribute( "type" , "button" ) //
            btnConfidence.setAttribute( "id" , "[Prediction Confidence]" ) //
            // btnConfidence.innerHTML = "How confident is the model for this prediction?"
            btnConfidence.innerHTML = "Prediction Confidence"


            var btnExample                     = document.createElement("button");
            btnExample.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            btnExample.setAttribute( "type" , "button" ) //
            btnExample.setAttribute( "id" , "[Similar Examples]" ) //
            // btnExample.innerHTML = "What are the most similar examples in the trainset?"
            // btnExample.innerHTML = "What are some published sentences that look similar to mine semantically?"
            btnExample.innerHTML = "Similar Published Sentences"

            var btnAttribution                     = document.createElement("button");
            btnAttribution.setAttribute( "class" , "reply xaiquestionButton" )  //acl
            ...
            btnCounterfactual.setAttribute( "class" , "reply xaiquestionButton" )  //acl
        ...
        case "long"    :
            ...
            btnLength.setAttribute( "type" , "button" ) //
            btnLength.setAttribute( "id" , "[Sentence Length]" ) //
            // btnLength.innerHTML = "What's the statistics of the sentence lengths?"
            btnLength.innerHTML = "Sentence Length"
        ...
        case "quality"    :
            ...
            qualityScore.setAttribute( "type" , "button" ) //
            qualityScore.setAttribute( "id" , "[Quality Score Range]" ) //
            // qualityScore.innerHTML = "What's the range of the style quality score?"
    ...
    //chatBotSession.lastChild.scrollIntoView(false);
    // Ruchi. Add
    //lastReplyContainer.scrollIntoView({behavior: "smooth", block: "end", inline: "nearest"})
    //$(document).find(".chatSession")[0];
}
...
function handleExplanationButtonClicked(e) {
    ...
    if (ButtonID == "[Data Statistics]") {
        ...
        // chatBotTextArea.value = "What are the most similar examples in the trainset?";
        ...
        chatBotTextArea.value = "Can you explain this sentence review?";
    }
    else{
...
$(".select-sentence-hover").hover(function() {
...
// ********** Chat Scrolling ********** //
//The handling for button clicked in chat history is handled in the click listener for the chat send button
window.setInterval(() => {
    if(!userHasScrolled) {
        chatBotSession.scrollTop = chatBotSession.scrollHeight;                                     //Scrolling to the bottom
    }
},300)

//Handling user scrolls
...
// // Show me All Button
// /*****************************************/
// $("#explanation-button-all").click(function(evt) {
//     ButtonID = "[ShowAll]"
//     chatBotTextArea             = document.querySelector( ".chatBot .chatForm #chatTextBox" )
//     chatBotTextArea.value = "What are the questions I can ask to explain this sentence?";
//     return;
//   }
// )

// $("#explanation-button-global").click(function(evt) {
//     ButtonID == "[GlobalWhy]"
//     chatBotTextArea             = document.querySelector( ".chatBot .chatForm #chatTextBox" )
//     chatBotTextArea.value = "Can you show me some general information about the model and data?";
//     return;
//   }
// )

// $("#explanation-button-instance").click(function(evt) {
//     ButtonID = "[InstanceWhy]"
//     chatBotTextArea             = document.querySelector( ".chatBot .chatForm #chatTextBox" )
//     chatBotTextArea.value = "Why is this sentence given this prediction and review?";
//     return;
//   }
// )
...
// $(".xaiquestionButton").click(function(evt) {
//     console.log("evt", evt)
//     ButtonID = "[InstanceWhy]"
//     chatBotTextArea             = document.querySelector( ".chatBot .chatForm #chatTextBox" )
//     chatBotTextArea.value = "Why is this sentence given this prediction and review?";
//     // explanationMessage = chatBotTextArea.value
//     console.log("===>>>chatBotTextArea", chatBotTextArea.value)
//     return;
//   }
// )
...
//     // chatBotSession.scrollTop + chatBotSession.offsetHeight;
//     console.log("chatBotSession.scrollTop", chatBotSession.scrollTop);
//     console.log("chatBotSession.offsetHeight", chatBotSession.offsetHeight);
// }



// chatBotSession.scrollTop = chatBotSession.scrollHeight;
// chatBotSession.animate({scrollTop: chatBotSession.scrollTop+chatBotSession.offsetHeight});
    ...
    //     chatBotReplys  = data.text
    //     global_xai = chatBotReplys[0]
    //     local_xai    = chatBotReplys[1]
    //     chatBotGlobalXAIButtonMessage = chatBotReplys[0]
    //     chatBotLocalXAIButtonMessage = chatBotReplys[1]
    //     var typeOfContainer = "showall-explanation"
    //     createContainer(typeOfContainer)
    // }